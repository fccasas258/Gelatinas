...
function displayCartProduct() {
    ...
    cart.forEach((item) => {
        ...
        <tr class="cart-item">
            ...
            <td>${item.name}</td>
            <td>Bs. ${item.price.newPrice.toFixed(2)}</td>
            <td>${item.quantity}</td>
            <td>Bs. ${(item.price.newPrice * item.quantity).toFixed(2)}</td>
        </tr>
        `
    })
...
function saveCardValues() {
    ...
    subTotal.innerHTML = `Bs. ${itemsTotal.toFixed(2)}`
    cartTotal.innerHTML = `Bs. ${itemsTotal.toFixed(2)}`
    fastCargo.addEventListener("change", (e) => {
        if (e.target.checked) {
            cartTotal.innerHTML = `Bs. ${(itemsTotal + fastCargoPrice).toFixed(2)}`
        } else {
            cartTotal.innerHTML = `Bs. ${itemsTotal.toFixed(2)}`
        }
    })
